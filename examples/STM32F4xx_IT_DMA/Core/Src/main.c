...
int main(void)
{
  ...
  //cuando resetea el  dispositivo pongo D+ y D- del USB en bajo para que Windows lo re-enumere
      ...
      GPIO_InitStruct = (LL_GPIO_InitTypeDef){0};
...
void DMA1_Stream3_IRQHandler(void){
	...
	extern volatile uint8_t *write;
	extern volatile uint8_t *send;
	extern volatile int samples;
	extern volatile uint8_t buffer0[];
	extern volatile uint8_t buffer1[];
	#define MAX_SAMPLES 64*10

	GPIOD->BSRR = GPIO_PIN_12;

	uint8_t *pbuff = (uint8_t *)( write + (samples * ADS_BYTES_PER_SAMPLE) );
	ads_interface_spi_rx_sample_DMA_prepare_next(&ads_device, pbuff);
	samples++;
...
void EXTI4_IRQHandler(void){
	...
	GPIOD->BSRR = GPIO_PIN_12;
	  ...
	  if (LL_EXTI_IsActiveFlag_0_31(LL_EXTI_LINE_4) != RESET)
	  {
	    ...
	    ads_interface_spi_rx_sample_DMA(&ads_device);
