# ADS1299 Polling Example for STM32F407

This project provides an example of using the `ads1299` library via polling on an STM32F407 microcontroller.
...
5.  **Main Loop:** In the infinite loop, the program polls the `/DRDY` pin. When a new sample is ready, it reads and stores the data in a ping-pong buffer perfectly sized to stream appropriately over USB CDC.
...
*   **`/DRDY`:** PC4 configured as an input.
...
The overall CPU utilization is practically entirely caused by reading the samples:
*   **66% CPU utilization** when compiled with the `-O0` flag.
*   **28% CPU utilization** when compiled with the `-Og` flag.

### Logic Analyzer Captures

**Polling with `-O0` Optimization (Overall View):**
![Polling -O0 Capture 1](polling_O0.png)

**Polling with `-O0` Optimization (Zoomed View):**
![Polling -O0 Capture 2](polling_O0_2.png)

**Polling with `-Og` Optimization:**
![Polling -Og Capture](polling_Og.png)
